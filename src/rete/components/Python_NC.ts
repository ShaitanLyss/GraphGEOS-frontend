...
`
    ...
    static collectPythonData(
        ...
        while ((match = pattern.exec(codeTemplate)) !== null) {
            ...
            const key = match[2];
            if (key === "this") {
                templateVars[key] = node.pythonComponent.code.map((code) => childIndentation + code).join("\n");
            } else {
                const outgoer = node.getOutgoer(key.replace("_", "-"));
                const { importsStatements, code } = PythonNodeComponent.collectPythonData(outgoer, childIndentation, null, allVars);
                console.log("Before merge", resImportsStatements, importsStatements)
                // Merge imports statements
                resImportsStatements = new Set(function* () { yield* resImportsStatements; yield* importsStatements; }());
