...
export function ConnectionWrapper(props: Props) {
	...
	useEffect(() => {
		const unwatch1 =
			typeof props.start === 'function' && props.start((s) => flush.apply(() => setStart(s)));
		const unwatch2 =
			typeof props.end === 'function' && props.end((s) => flush.apply(() => setEnd(s)));
	...
	return (
		<ConnectionContext.Provider value={{ start, end, path }}>{children}</ConnectionContext.Provider>
	);
}
