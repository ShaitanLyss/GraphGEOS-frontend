import { CreateAccountStore, CreateSessionStore, CreateUserStore, DeleteSessionStore, SessionAndUserStore, UserAuthInfoStore, UserCreateInput } from "$houdini";
...
export function GraphQlAdapter(event: RequestEvent): Adapter {
    return {
        ...
        async getUserByAccount(
            ...
            provider = provider;
            const userAuthInfo = new UserAuthInfoStore();
            const user = (await userAuthInfo.fetch({ event, variables: { provider: provider } })).data?.user;
            if (user == undefined) {
                return null;
            }
            return user;
        ...
        async getSessionAndUser(
            ...
            const data = (await sessionAndUser.fetch({ event, variables: { sessionToken: sessionToken } })).data;
