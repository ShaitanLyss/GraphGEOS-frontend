...
type CustomizationProps<Schemes extends ClassicScheme> = {
	...
	control?: <N extends ClassicPreset.Control>(
				...
				return (
					component && {
						...
						props: {
							...
							start:
								context.data.start ||
								((change: any) => positionWatcher.listen(source, 'output', sourceOutput, change)),
							end:
								context.data.end ||
								((change: any) => positionWatcher.listen(target, 'input', targetInput, change)),
